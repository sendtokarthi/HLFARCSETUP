...
function formatDate(dateString) {
    ...
    // Replace any instances of "SEPT" with "SEP" to match desired format
    formattedDate = formattedDate.replace('SEPT', 'SEP');

    // Replace spaces with hyphens to achieve the desired format "09-SEP-2023"
...
router.post('/view-table', async (req, res) => {
    ...
    try {
        ...
        res.render('view-table', { rows: dataResult.rows, columns, table, formatDate });
    } catch (err) {
        console.error(err);
        res.redirect('/login');
    }
...
router.get('/module-form', async (req, res) => {
    if (!req.session.user) {
        return res.redirect('/login');
    }
    ...
    try {
        ...
        await connection.close();
        res.render('module-form', { modules: result.rows });
    } catch (err) {
        res.redirect('/login');
    }
